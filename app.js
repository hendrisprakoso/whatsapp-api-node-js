...
app.post('/fetch-message', async (req, res) => {
	...
	const chat = await client.fetchMessages(number);
	...
	res.status(200).json({
		status: true,
		message: 'masuk fetch message!'
	});
	
});

// Check number is regitered in Whatsapp 
app.post('/number-registred', async (req, res) => {

	const vNumber = phoneNumberFormatter(req.body.number);
	const isRegisteredNumber = await checkRegisteredNumber(vNumber);
...
app.post('/status-online', async (req, res) => {
	...
	console.log('client : ', client.authStrategy.client);


	res.status(200).json({
		status: false,
		message: "Masuk ke endpoint /status-online !"
	});
